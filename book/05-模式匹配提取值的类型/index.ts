...
type GetArrayFirstType<A extends unknown[]> = A extends [
  infer First,
  ...unknown[],
]
  ? First
...
type GetArrayLastType<A extends unknown[]> = A extends [
  ...unknown[],
  infer Last,
]
  ? Last
...
type startWith<
  Str extends string,
  Pirfix extends string,
> = Str extends `${Pirfix}${string}` ? true : false;
...
type ReplaceStr<
  ...
  From extends string,
  To extends string,
> = Str extends `${infer Pirfix}${From}${infer Suffix}`
  ? `${Pirfix}${To}${Suffix}`
  : Str;
 ...
 */
// 提取函数参数类型
type GetParamType<F extends Function> = F extends (
  ...args: infer Args
) => unknown
  ? Args
  : never;
...
type GetReturnType<F extends Function> = F extends (...args: any) => infer R
  ? R
  : never;
