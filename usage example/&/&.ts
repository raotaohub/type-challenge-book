...
type TupleToUn<T extends unknown[], V = never> = T extends [
  infer Val,
  ...infer Rest,
]
...
// good
type IFooog = Partial<Pick<IFoog, 'A'>> &
  Pick<IFoog, 'B'> &
  Readonly<Pick<IFoog, 'C'>>;
...
// bad
type IFoooob = Partial<Pick<IFoog, 'A'>> &
  Pick<AbstractInterface, ZZZ> &
  Pick<IFoog, 'B'> &
  Readonly<Pick<IFoog, 'C'>>;
type read1 = Read<IFoooob>;

// good
type IFoooog = Omit<AbstractInterface, 'Z' | 'C'> &
  Pick<IFoog, 'B'> &
  Readonly<Pick<IFoog, 'C'>>;
...
type IFoooog2 = Readonly<Pick<AbstractInterface, ReadonlyUn>> &
