...
type ParseParam<Str extends string> = Str extends `${infer Key}=${infer Val}` ? { [K in Key]: Val } : {}

type MergeParam<Result extends Record<string, string>, OtherResult extends Record<string, string>> = {
...
type ParseString<Str extends string> = Str extends `${infer Param}&${infer Rest}`
    ? MergeParam<ParseParam<Param>, ParseString<Rest>>
    : ParseParam<Str>

type ParseQueryString<Str extends string> = Str extends `${infer Url}?${infer Qs}`
    ? ParseString<Qs>
    : ParseString<Str>
